...
static void __iomem *msm_ioremap_size(struct platform_device *pdev,
				      ...
				      phys_addr_t *size)
{
	...
	if (!res) {
		DRM_DEV_ERROR(&pdev->dev, "resource '%s' not found\n", dbgname);
...
static bool dsi_phy_7nm_is_7nm(const struct msm_dsi_phy_cfg *cfg)
{
        return (cfg == &dsi_phy_7nm_cfgs ||
                cfg == &dsi_phy_7nm_8150_cfgs ||
                cfg == &dsi_phy_7nm_7280_cfgs);
}

static bool dsi_phy_is_7nm(const struct msm_dsi_phy_cfg *cfg)
{
        return (cfg == &dsi_phy_7nm_cfgs ||
                cfg == &dsi_phy_7nm_8150_cfgs ||
                cfg == &dsi_phy_7nm_7280_cfgs);
...
static int dsi_phy_driver_probe(struct platform_device *pdev)
{
        ...
        /* Allocate provided_clocks for 7nm PHY */
        if (dsi_phy_7nm_is_7nm(phy->cfg)) {
        ...
        if (phy->cfg->has_phy_regulator) {
                ...
                if (IS_ERR(phy->reg_base)) {
                        DRM_DEV_ERROR(dev, "%s: failed to map phy regulator base\n", __func__);
                        ret = -ENOMEM;
                        goto fail;
                }
        }
	...
	if (dsi_phy_is_7nm(phy->cfg)) {
		/* 7nm PHY: use new pll_init ops */
		...
		/* Older PHYs: use the old msm_dsi_pll_init path */
			...
			struct msm_dsi_phy_shared_timings *shared_timings)
{
	 ...
	 * Restore PLL status if the PLL is being used as clock source.
	 */
	if (phy->usecase != MSM_DSI_PHY_SLAVE) {
		if (dsi_phy_is_7nm(phy->cfg)) {
			/* 7nm: use new state save/restore */
			ret = msm_dsi_phy_pll_restore_state(phy);
			if (ret) {
				DRM_DEV_ERROR(dev,
					"%s: failed to restore phy state, %d\n",
					__func__, ret);
				goto pll_restor_fail;
			}
		} else if (phy->pll) {
			/* Older PHYs: use old msm_dsi_pll_restore_state */
...
{
        if (byte_clk_provider && phy->provided_clocks)
                *byte_clk_provider = phy->provided_clocks->hws[DSI_BYTE_PLL_CLK]->clk;
        if (pixel_clk_provider && phy->provided_clocks)
