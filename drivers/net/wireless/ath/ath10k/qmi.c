				 ...
				 struct qmi_service *service)
{
	...
	ath10k_err(ar, "QMI: WLFW service found at node=%u port=%u version=%u instance=%u\n",
		   service->node, service->port, service->version, service->instance);
	ath10k_dbg(ar, ATH10K_DBG_QMI, "wifi fw qmi service found\n");

	ath10k_err(ar, "QMI: Attempting to connect to service at node=%u port=%u\n",
		   sq->sq_node, sq->sq_port);
	ret = kernel_connect(qmi_hdl->sock, (struct sockaddr *)&qmi->sq,
			     sizeof(qmi->sq), 0);
	if (ret) {
		ath10k_err(ar, "QMI: failed to connect to remote QMI service: %d\n", ret);
		return ret;
	}

	ath10k_err(ar, "QMI: Successfully connected to WLFW service\n");
...
int ath10k_qmi_init(struct ath10k *ar, u32 msa_size)
{
	...
	ath10k_err(ar, "QMI: Initializing QMI handle (max_msg_len=%zu)\n",
		   (size_t)WLFW_BDF_DOWNLOAD_REQ_MSG_V01_MAX_MSG_LEN);
	ret = qmi_handle_init(&qmi->qmi_hdl,
			      WLFW_BDF_DOWNLOAD_REQ_MSG_V01_MAX_MSG_LEN,
			      &ath10k_qmi_ops, qmi_msg_handler);
	if (ret) {
		ath10k_err(ar, "QMI: Failed to initialize QMI handle: %d\n", ret);
		goto err;
	}
	ath10k_err(ar, "QMI: QMI handle initialized successfully\n");
	...
	ath10k_err(ar, "QMI: Registering for WLFW service (ID=0x%x version=0x%x)\n",
		   WLFW_SERVICE_ID_V01, WLFW_SERVICE_VERS_V01);
	ret = qmi_add_lookup(&qmi->qmi_hdl, WLFW_SERVICE_ID_V01,
			     WLFW_SERVICE_VERS_V01, 0);
	if (ret) {
		ath10k_err(ar, "QMI: Failed to register service lookup: %d\n", ret);
		goto err_qmi_lookup;
	}
	ath10k_err(ar, "QMI: Waiting for WLFW service to appear...\n");
