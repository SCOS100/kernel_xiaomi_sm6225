...
{
	struct clk_devm_free *f = res;
	f->unregister(f->clk);
}

static struct clk *__devm_clk_register(struct device *dev,
				       struct clk *clk,
				       void (*unregister)(struct clk *clk))
{
	struct clk_devm_free *f;

	if (IS_ERR(clk))
		return clk;

	f = devres_alloc(devm_clk_release, sizeof(*f), GFP_KERNEL);
	if (!f) {
		unregister(clk);
...
static struct clk *devm_clk_register_divider(struct device *dev,
		const char *name, const char *parent_name,
		unsigned long flags, void __iomem *reg, u8 shift, u8 width,
		u8 clk_divider_flags, spinlock_t *lock)
{
	struct clk *clk;
	clk = clk_register_divider(dev, name, parent_name, flags,
				   reg, shift, width, clk_divider_flags, lock);
	return __devm_clk_register(dev, clk, (void (*)(struct clk *))clk_unregister_divider);
}

static struct clk *devm_clk_register_fixed_factor(struct device *dev,
		const char *name, const char *parent_name,
		unsigned long flags, unsigned int mult, unsigned int div)
{
	struct clk *clk;
	clk = clk_register_fixed_factor(dev, name, parent_name,
					 flags, mult, div);
	return __devm_clk_register(dev, clk, (void (*)(struct clk *))clk_unregister);
}

static struct clk *devm_clk_register_mux(struct device *dev,
		const char *name, const char * const *parent_names,
		u8 num_parents, unsigned long flags, void __iomem *reg,
		u8 shift, u8 width, u8 clk_mux_flags, spinlock_t *lock)
{
	struct clk *clk;
	clk = clk_register_mux(dev, name, parent_names, num_parents,
			       flags, reg, shift, width, clk_mux_flags, lock);
	return __devm_clk_register(dev, clk, (void (*)(struct clk *))clk_unregister_mux);
}
...
/* forward declaration – available inside the clock core, but not exported to modules.
 * If you get an undefined reference, either include clk/clk.h (if built‑in) or
 * add this declaration exactly as shown.
 */
extern struct clk_hw *__clk_get_hw(struct clk *clk);
...
static int pll_7nm_register(struct dsi_pll_7nm *pll_7nm, struct clk_hw **provided_clocks)
{
	...
	const char *vco_parents[] = { "ref" };
	struct clk_init_data vco_init = {
		.parent_names = vco_parents,
		.num_parents = 1,
		.name = vco_name,
	...
	clk = devm_clk_register(dev, &pll_7nm->clk_hw);
	if (IS_ERR(clk)) {
		ret = PTR_ERR(clk);
		DRM_DEV_ERROR(dev, "failed to register vco clk: %d\n", ret);
		return ret;
	}
	...
	clk = devm_clk_register_divider(dev, clk_name, parent,
				     CLK_SET_RATE_PARENT,
				     pll_7nm->phy->pll_base +
				     REG_DSI_7nm_PHY_PLL_PLL_OUTDIV_RATE,
				     0, 2, CLK_DIVIDER_POWER_OF_TWO, NULL);
	if (IS_ERR(clk)) { ret = PTR_ERR(clk); goto fail; }
	...
	clk = devm_clk_register_fixed_factor(dev, clk_name, parent,
					  CLK_SET_RATE_PARENT, 1,
					  pll_7nm->phy->cphy_mode ? 7 : 8);
	if (IS_ERR(clk)) { ret = PTR_ERR(clk); goto fail; }
	provided_clocks[DSI_BYTE_PLL_CLK] = __clk_get_hw(clk);
	...
	if (pll_7nm->phy->cphy_mode)
		clk = devm_clk_register_fixed_factor(dev, clk_name, parent, 0, 2, 7);
	else
		clk = devm_clk_register_fixed_factor(dev, clk_name, parent, 0, 1, 4);
	if (IS_ERR(clk)) { ret = PTR_ERR(clk); goto fail; }

	/* pclk_mux (or bypass for C‑PHY) */
	if (pll_7nm->phy->cphy_mode) {
		u32 data;
		...
		clk = devm_clk_register_mux(dev, clk_name,
					((const char *[]){
	...
	clk = devm_clk_register_divider(dev, clk_name, parent,
					...
					REG_DSI_7nm_PHY_CMN_CLK_CFG0,
				     ...
				     &pll_7nm->postdiv_lock);
	if (IS_ERR(clk)) { ret = PTR_ERR(clk); goto fail; }
	provided_clocks[DSI_PIXEL_PLL_CLK] = __clk_get_hw(clk);
...
/* Initialize PLL baseline register values - required for proper PHY operation */
static void dsi_7nm_pll_init_baseline(struct msm_dsi_phy *phy)
{
	void __iomem *pll_base = phy->pll_base;

	/* Critical PSM/RSM and core control setup */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PSM_CTRL, 0x00000020);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_RSM_CTRL, 0x00000010);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_VCO_TUNE_MAP, 0x00000002);

	/* PLL main control settings */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_CNTRL, 0x0000001C);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_SYSTEM_MUXES, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_INT_LOOP_SETTINGS, 0x0000003F);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_INT_LOOP_CONTROLS, 0x00000080);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_ANALOG_CONTROLS_ONE, 0x00000000);

	/* Band select calibration */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BAND_SEL_MIN, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BAND_SEL_MAX, 0x000000FF);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BAND_SEL_CAL_TIMER_LOW, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BAND_SEL_CAL_TIMER_HIGH, 0x00000002);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BAND_SEL_CAL_SETTINGS, 0x00000020);

	/* Frequency detection */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FREQ_DETECT_THRESH, 0x00000020);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FREQ_DET_REFCLK_HIGH, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FREQ_DET_REFCLK_LOW, 0x000000FF);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FREQ_DET_PLLCLK_HIGH, 0x00000010);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FREQ_DET_PLLCLK_LOW, 0x00000046);

	/* Lock detection */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_LOCKDET, 0x00000040);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FASTLOCK_CONTROL, 0x00000004);

	/* PLL gain and bias */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_GAIN, 0x00000054);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BIAS_CONTROL_1, 0x00000040);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BIAS_CONTROL_2, 0x00000020);

	/* Filter settings */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BAND_SEL_PFILT, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_BAND_SEL_IFILT, 0x0000000A);

	/* SSC setup (Spread Spectrum Clock) - disabled by default */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_SSC_STEPSIZE_LOW, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_SSC_STEPSIZE_HIGH, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_SSC_DIV_PER_LOW, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_SSC_DIV_PER_HIGH, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_SSC_ADJPER_LOW, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_SSC_ADJPER_HIGH, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_SSC_CONTROL, 0x00000000);

	/* Core override and input settings */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_CORE_OVERRIDE, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_CORE_INPUT_OVERRIDE, 0x00000010);

	/* FLL (Frequency Locked Loop) config */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FLL_CONFIG, 0x00000002);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FLL_FREQ_ACQ_TIME, 0x00000011);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FLL_GAIN0, 0x00000080);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_FLL_GAIN1, 0x00000000);

	/* Rate configuration */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_OUTDIV_RATE, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_PROP_GAIN_RATE_2, 0x0000000C);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_BAND_SEL_RATE_2, 0x00000000);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_INT_GAIN_IFILT_BAND_2, 0x00000054);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_FL_INT_GAIN_PFILT_BAND_2, 0x0000004C);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_FASTLOCK_EN_BAND, 0x00000003);

	/* Power down controls */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PS3_PWRDOWN_CONTROLS, 0x0000001D);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PS4_PWRDOWN_CONTROLS, 0x0000001C);

	/* Enable lock override and delay */
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_LOCK_OVERRIDE, 0x00000080);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_LOCK_DELAY, 0x00000006);
	dsi_phy_write(pll_base + REG_DSI_7nm_PHY_PLL_PLL_LOCK_MIN_DELAY, 0x00000019);
}
			      ...
			      struct msm_dsi_phy_clk_request *clk_req)
{
	...
	/* Initialize PLL baseline register values */
	dsi_7nm_pll_init_baseline(phy);
